...
static fp32 rotate_ratio_f = ((Wheel_axlespacing + Wheel_spacing) / 2.0f - GIMBAL_OFFSET)*1.6; //rad 0.4195左右
static fp32 rotate_ratio_b = ((Wheel_axlespacing + Wheel_spacing) / 2.0f + GIMBAL_OFFSET)*1.6;//0.4195左右
...
_Noreturn void chassis_task(void const *pvParameters) {
    ...
    while (1) {
        ...
        if (chassis.mode != CHASSIS_RELAX) {
            //底盘最大速度控制，电容开关分别对应两种最大速度限制
            //chassis_vector_powerControl();
            //底盘解算
            chassis_wheel_cal();
            //功率限制
            if(detect_list[DETECT_CAP].status == ONLINE)
            {
                buffer_limit = Cap.capFeedback.esr_v > 2400 ? 50 - (((fp32) Cap.capFeedback.esr_v) / 2400 - 1) * 4 : 0;
//                k = ((fp32)Cap.capFeedback.esr_v - 2400) / 400 > 0 ? ((fp32)Cap.capFeedback.esr_v - 2400 / 400) * 0.3 : 0;
//                if(Cap.capFeedback.esr_v > 1500)
//                    chassis_power_limit(buffer_limit, 0.2);
//                else
//                    chassis_power_limit(50, 0);
                k = Cap.capFeedback.esr_v > 1500 ? 0.2 : (fp32)(Cap.capFeedback.esr_v - 1200) / 1500 * 0.2;
                chassis_power_limit(buffer_limit, k);
            }
            else if(detect_list[DETECT_CAP].status == OFFLINE)
            {
                chassis_power_limit(50, 0);
            }
            //驱电机闭环
...
void chassis_power_limit(fp32 buffer_limit, fp32 k) {
    ...
    chassis.chassis_power_limit.power_buff = Referee.PowerHeatData.chassis_power_buffer;
...
static float chassis_speed_change() {
    ...
    return speed_change+0.002f;
//    return speed_change;
}

bool a1,a2,a3,cap_on=false;
float m=1,n=1,decrease=0,decline=0;
static void chassis_vector_powerControl(){
//m控制vx vy最大值 n控制vw最大值
//m越大vx ，vy被限制越狠
//n越小 vw被限制越狠
    a1=(KeyBoard.SHIFT.status==KEY_PRESS||rc_ctrl.rc.s[1]==2);
    a2=(detect_list[DETECT_CAP].status==ONLINE);
    a3=(Cap.capFeedback.esr_v/100.0>16);
    decrease=0.2f;
    cap_on=a1&&a2&&a3;
        switch (Referee.GameRobotStat.chassis_power_limit) {
            ...
            case 100: {
                if(cap_on){
                    m=0.45+decline,n=5.8;
                }
                else{
                    m = 1.42+decrease, n = 4.0;
                }
    ...
    VAL_LIMIT(chassis.vx,-(MAX_CHASSIS_VX_SPEED-m),(MAX_CHASSIS_VX_SPEED-m))
    VAL_LIMIT(chassis.vy,-(MAX_CHASSIS_VY_SPEED-m),(MAX_CHASSIS_VY_SPEED-m))
    VAL_LIMIT(chassis.vw,-n,n);
    //操作手主动慢速
    if(KeyBoard.CTRL.status==KEY_PRESS)
    {
        VAL_LIMIT(chassis.vx,-MAX_CHASSIS_VX_SPEED*0.2,MAX_CHASSIS_VX_SPEED*0.2);
